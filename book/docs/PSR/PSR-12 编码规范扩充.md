...
> 一个 PHP 文件的头部 **可能** 会包含多个块。如果包含多个块，则每个块都 **必须** 用空白行和其他块分隔，并且块内 **不能** 包含空白行。所有的块都 **必须** 按照下面的顺序排列，如果不存在该块则忽略。
>
> * PHP 文件开始标签 `<?php`
> * 文件级文档块
> * 一个或多个声明语句
...
> `implements` 的实现列表 **可以** 分成多行，这样的话，第一个接口 **必须** 些在下一行，且每行 **必须** 只能写一个接口。
...
> 每个使用的 Trait **必须** 包含一个声明，且每个包含声明 **必须** 有其导入语句。
...
> 当使用 `insteadof` 和 `as` 运算符时，它们必须如下所示使用，注意缩进、间距和另起一行。
...
> 如果接口列表不换行，左花括号 **可以** 和关键字 `class` 在同一行，如果接口列表换行，花括号 **必须** 放在最后一个接口的下一行。
